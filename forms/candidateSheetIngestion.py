...
def candidateSheetIngestionWithAgent(jobId, candidate_sheet):
    ...
    files=[
    ('candidate_sheet',(candidate_sheet.name, candidate_sheet,'text/csv'))
    ]
...
def showJobsOnScreenAndReturnJobId():
    GET_API_URL = f"{LOCAL_URL}api/external-communications/get-active-jobs/"
    response = requests.get(GET_API_URL)
    if response.status_code == 200:
        response_dict = response.json()  # Assuming data is a dictionary with three elements
    ...
    selected_option = st.radio(
        label="Choose one",
        options=[(active_job["role"], active_job["hr_name"], active_job["jobId"]) for active_job in active_job_list],  # Assuming each value has an '_id'
        format_func=lambda x: showcase_template.format(x[0], x[1])  # Show only key names in UI
    ) 

    return selected_option

def ingestCandidateSheet():
    ...
    selected_option = showJobsOnScreenAndReturnJobId()
    jobId = selected_option[2]

    st.header("okay now upload your candidate sheet")

    # csv upload
    uploaded_candidate_sheet = st.file_uploader("Upload candidate sheet", type=["csv"], accept_multiple_files=False)
    ...
    if uploaded_candidate_sheet is not None:
        ...
        if st.button("Submit"):
            ...
            if response == None:
                logging.info(f"some error occured while uploading the candidate to agent for jobId: {jobId}") 
                st.write("candidate sheet ingestion failed sorry we will reach shortly or if you can you can write us at yashtiwari1906@gmail.com")
                return

            if response.status_code == 400:
                logging.info(f"some error happened but problem was at the user side for jobId: {jobId}")
