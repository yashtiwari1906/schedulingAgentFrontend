...
def showSlotSelectOptions():
    ...
    for day, slots in data[month_week1].items():
        ...
        with st.expander(day.capitalize()):
            ...
            selected = st.multiselect(f"Select time slots for {day.capitalize()}",
                                    options=slots,
                                    key=day)
            if selected:
...
def saveEmployeeAvailabilitySlotsPage():
    ...
    print(query_params)
    client_name, jobId = query_params.get("client_name", None) , query_params.get("jobId", None) 
    st.header("slot preference form")

    # Example form inputs
    employee_email = st.text_input("Employee email")
    if client_name is None and jobId is None:
    ...
    if st.button("Submit"):
        response = saveEmployeeAvailabilitySlotsWithAgent(jobId, employee_email, selected_slots)
        if response.status_code == 200:
            logging.info(f"response from the createJob API hit for jobId: {jobId} the employee_email: {employee_email}")
            st.write(f"Thanks for submitting")
        elif response.status_code == 400:
            logging.info(f"some error happened but problem was at the user side for jobId: {jobId} the employee_email: {employee_email}")
            ...
            logging.info(f"some internal error occured for the jobId: {jobId} the employee_email: {employee_email}")
